...
public class ModelBlueJay extends AdvancedEntityModel<EntityBlueJay> {
    private final AdvancedModelBox root,body,leftLeg,rightLeg,tail,leftWing,rightWing,head,crest;

    public ModelBlueJay(){texWidth=64;texHeight=64;root=new AdvancedModelBox(this,"root");root.setPos(0.0F,24.0F,0.0F);
        body=new AdvancedModelBox(this,"body");body.setPos(0.0F,-3.2F,0.0F);root.addChild(body);setRotateAngle(body,-0.1309F,0.0F,0.0F);body.setTextureOffset(0,0).addBox(-2.0F,-4.0F,-4.0F,4.0F,4.0F,7.0F,0.0F,false);
        leftLeg=new AdvancedModelBox(this,"leftLeg");leftLeg.setRotationPoint(1.5F,0.0F,1.0F);body.addChild(leftLeg);setRotateAngle(leftLeg,0.1309F,0.0F,0.0F);leftLeg.setTextureOffset(26,10).addBox(-1.5F,0.0F,-2.0F,3.0F,3.0F,2.0F,0.0F,false);
        rightLeg=new AdvancedModelBox(this,"rightLeg");rightLeg.setRotationPoint(-1.5F,0.0F,1.0F);body.addChild(rightLeg);setRotateAngle(rightLeg,0.1309F,0.0F,0.0F);rightLeg.setTextureOffset(26,10).addBox(-1.5F,0.0F,-2.0F,3.0F,3.0F,2.0F,0.0F,true);
        tail=new AdvancedModelBox(this,"tail");tail.setRotationPoint(0.0F,-3.0F,3.0F);body.addChild(tail);tail.setTextureOffset(0,22).addBox(-1.5F,0.0F,0.0F,3.0F,1.0F,7.0F,0.0F,false);
        leftWing=new AdvancedModelBox(this,"leftWing");leftWing.setRotationPoint(2.0F,-3.0F,-2.0F);body.addChild(leftWing);leftWing.setTextureOffset(15,14).addBox(0.0F,-1.0F,-1.0F,1.0F,3.0F,8.0F,0.0F,false);
        rightWing=new AdvancedModelBox(this,"rightWing");rightWing.setRotationPoint(-2.0F,-3.0F,-2.0F);body.addChild(rightWing);rightWing.setTextureOffset(15,14).addBox(-1.0F,-1.0F,-1.0F,1.0F,3.0F,8.0F,0.0F,true);
        head=new AdvancedModelBox(this,"head");head.setRotationPoint(0.0F,-3.0F,-4.0F);body.addChild(head);setRotateAngle(head,0.2182F,0.0F,0.0F);head.setTextureOffset(23,0).addBox(-2.5F,-3.0F,-3.0F,5.0F,4.0F,5.0F,0.0F,false);head.setTextureOffset(26,16).addBox(-1.0F,-1.0F,-6.0F,2.0F,2.0F,3.0F,0.0F,false);
        crest=new AdvancedModelBox(this,"crest");crest.setRotationPoint(0.0F,-2.0F,-1.0F);head.addChild(crest);setRotateAngle(crest,0.3491F,0.0F,0.0F);crest.setTextureOffset(0,12).addBox(-2.5F,-1.0F,0.0F,5.0F,3.0F,6.0F,-0.01F,false);
        this.updateDefaultPose();}

    @Override public Iterable<BasicModelPart> parts(){return ImmutableList.of(root);}
    @Override public Iterable<AdvancedModelBox> getAllParts(){return ImmutableList.of(root,body,crest,head,tail,leftLeg,rightLeg,leftWing,rightWing);}
    ...
    public void setupAnim(EntityBlueJay entity,float limbSwing,float limbSwingAmount,float ageInTicks,float netHeadYaw,float headPitch){
        ...
        float flapSpeed=0.65F,flapDegree=0.25F,walkSpeed=0.95F,walkDegree=0.6F,idleSpeed=0.12F,idleDegree=0.12F;
        ...
        //══════ 🐦 BLUE JAY — RAUCOUS FOREST BULLY ══════
        // IDENTITY: Bossy, loud, aggressive. Crest raises/lowers with mood
        // (like mood ring). Quick nervous hop-walks. Loud wing snaps.
        // Sings with crest bobbing. Bullies other birds from feeders.
        // UNIQUE vs Toucan (fruit-tosser), Hummingbird (hoverer).

        // ── BREATHING: fast energetic ──
        float breath=Mth.cos(ageInTicks*0.17F);body.rotationPointY+=breath*0.15F;

        // ── CREST: mood indicator — raises when excited, lowers when calm ──
        progressRotationPrev(crest,crestAmount,Maths.rad(20),0,0,1F);
        this.walk(crest,idleSpeed,idleDegree,false,2F,0F,ageInTicks,1.0F);

        // ── WINGS: loud, showy flaps with wing snaps ──
        ...
        progressRotationPrev(head,biteProgress,Maths.rad(60),0,0,2.5F);
        ...
        this.walk(tail,idleSpeed,idleDegree,false,1F,0F,ageInTicks,1.0F);
        this.bob(head,idleSpeed,idleDegree*1.5F,true,ageInTicks,1.0F);

        this.faceTarget(netHeadYaw,headPitch,1.3F,head);
        this.body.rotateAngleX+=birdPitch*flyProgress*0.2F*Mth.DEG_TO_RAD;

        // ── FEEDING/SINGING: crest bobs when excited ──
        if(entity.getFeedTime()>0||entity.getSingTime()>0){
            ...
            head.rotationPointZ+=(float)(Math.sin(ageInTicks*-0.4-1.0F));
        }
    }

    public void renderToBuffer(PoseStack ms,VertexConsumer b,int l,int o,float r,float g,float bl,float a){
        if(this.young){float f=1.35F;head.setScale(f,f,f);head.setShouldScaleChildren(true);ms.pushPose();ms.scale(0.5F,0.5F,0.5F);ms.translate(0.0D,1.5D,0D);parts().forEach(p->p.render(ms,b,l,o,r,g,bl,a));ms.popPose();head.setScale(1F,1F,1F);}
